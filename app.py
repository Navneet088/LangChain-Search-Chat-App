...
# Use Arxiv, Wikipedia, and DuckDuckGo tools to search
...
if prompt := st.chat_input("Ask something"):
    ...
    search_agent = initialize_agent(
        ...
        memory=st.session_state["memory"],   # ✅ persistent memory across turns
        handle_parsing_errors=True            # ✅ fixed typo
    )

    with st.chat_message("assistant"):
        st_cb = StreamlitCallbackHandler(st.container(), expand_new_thoughts=False)
        response = search_agent.run(prompt, callbacks=[st_cb])  # ✅ pass prompt, not full history
        st.write(response)
